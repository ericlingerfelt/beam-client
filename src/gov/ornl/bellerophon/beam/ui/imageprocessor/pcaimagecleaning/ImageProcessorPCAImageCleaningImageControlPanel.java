...
public class ImageProcessorPCAImageCleaningImageControlPanel extends JPanel implements ActionListener, ChangeListener{
	...
	private ImageProcessorPCAImageCleaningImageControlPanelListener listener;
	private PCAImageType type;
	
	public ImageProcessorPCAImageCleaningImageControlPanel(ImageProcessorPCAImageCleaningImageControlPanelListener listener, PCAImageType type){
		...
		double[] columnBox = {5, TableLayoutConstants.PREFERRED, 5};
		double[] rowBox = {5, TableLayoutConstants.PREFERRED
						, 7, TableLayoutConstants.PREFERRED
						, 7, TableLayoutConstants.PREFERRED
						, 7, TableLayoutConstants.PREFERRED, 5};
		boxPanel.setLayout(new TableLayout(columnBox, rowBox));
		boxPanel.add(viewColorBarsBox, 			"1, 1, l, c");
		boxPanel.add(viewAxesBox, 				"1, 3, l, c");
		boxPanel.add(applySigmaBox, 			"1, 5, l, c");
		boxPanel.add(mapBox, 					"1, 7, l, c");
	...
	public void stateChanged(ChangeEvent ce) {
		if(ce.getSource()==zoomSpinner){
			listener.imageControlPanelZoomSizeChanged(type);
		}else if(ce.getSource()==sigmaSpinner){
			listener.imageControlPanelSigmaValueChanged(type);
		}
	}
	
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==viewColorBarsBox){
			listener.imageControlPanelShowColorBarsChanged(type);
		}else if(ae.getSource()==viewAxesBox){
			listener.imageControlPanelShowAxisChanged(type);
		}else if(ae.getSource()==showCrossHairsBox){
			listener.imageControlPanelShowCrossHairsChanged(type);
		}else if(ae.getSource()==colorMapBox){
			listener.imageControlPanelColorMapChanged(type);
			...
			listener.imageControlPanelApplySigmaChanged(type);
		}else if(ae.getSource()==mapBox){
			listener.imageControlPanelMapOutliersChanged(type);
		}
	}
