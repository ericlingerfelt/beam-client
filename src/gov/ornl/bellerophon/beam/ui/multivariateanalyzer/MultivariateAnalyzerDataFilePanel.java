...
import gov.ornl.bellerophon.beam.ui.multivariateanalyzer.kmeansclustering.MultivariateAnalyzerKMeansClusteringDataFileTreeCellRenderer;
import gov.ornl.bellerophon.beam.ui.multivariateanalyzer.pca.MultivariateAnalyzerPCADataFileTreeCellRenderer;
...
public class MultivariateAnalyzerDataFilePanel extends JPanel implements ActionListener, 
	...
	private TreeMap<MultivariateAnalyzerMode, DefaultTreeCellRenderer> dataFileTreeCellRendererMap;
	
	public MultivariateAnalyzerDataFilePanel(Frame frame, MultivariateAnalyzerData d, DataFileSelectionListener dfsl, DataFileTreeNodeSelectionListener dftnsl){
		
		this.frame = frame;
		this.d = d;
		this.dfsl = dfsl;
		
		dataFileTreeCellRendererMap = new TreeMap<MultivariateAnalyzerMode, DefaultTreeCellRenderer>();
		dataFileTreeCellRendererMap.put(MultivariateAnalyzerMode.PCA_ANALYSIS_MODE, new MultivariateAnalyzerPCADataFileTreeCellRenderer());
		dataFileTreeCellRendererMap.put(MultivariateAnalyzerMode.KMEANS_CLUSTERING_ANALYSIS_MODE, new MultivariateAnalyzerKMeansClusteringDataFileTreeCellRenderer());
		dataFileTreeCellRendererMap.put(MultivariateAnalyzerMode.BAYESIAN_ANALYSIS_MODE, new MultivariateAnalyzerPCADataFileTreeCellRenderer());
		dataFileTreeCellRendererMap.put(MultivariateAnalyzerMode.ICA_ANALYSIS_MODE, new MultivariateAnalyzerPCADataFileTreeCellRenderer());
		...
		selectLabel.setText("Please select a data file from your BEAM data storage area or upload a new data file to your"
				+ " BEAM data storage area for multivariate analysis.");
		...
		tree.setDataFileTreeRenderer(dataFileTreeCellRendererMap.get(MultivariateAnalyzerMode.PCA_ANALYSIS_MODE));
	...
	public void setCurrentState(){
		...
		if(dataFile==null){
			...
			if(mode==MultivariateAnalyzerMode.PCA_ANALYSIS_MODE && d.getDataFile().getPCADataSet()!=null && !d.getDataFile().getPCADataSet().getGroupPath().equals("")){
				selectPCAGroupTreeNode();
			}
			
			if(mode==MultivariateAnalyzerMode.KMEANS_CLUSTERING_ANALYSIS_MODE && d.getDataFile().getKMeansClusteringDataSet()!=null && !d.getDataFile().getKMeansClusteringDataSet().getGroupPath().equals("")){
				selectKMeansClusteringGroupTreeNode();
			}
	...
	private void selectPCAGroupTreeNode(){
		String groupPath = d.getDataFile().getPCADataSet().getGroupPath();
	...
	private void selectKMeansClusteringGroupTreeNode(){
		String groupPath = d.getDataFile().getKMeansClusteringDataSet().getGroupPath();
		DefaultMutableTreeNode groupNode = d.getDataFile().getDefaultMutableTreeNode(groupPath);
		tree.setSelectionPath(new TreePath(d.getDataFile().getTreeModel().getPathToRoot(groupNode)));
	}

	public void reloadTreeModelAfterPCA() {
		GetDataFileTreeWorker worker = new GetDataFileTreeWorker(this, d.getDataFile(), frame);
		worker.execute();
	}

	public void setMode(MultivariateAnalyzerMode mode) {
		this.mode = mode;
		tree.setDataFileTreeRenderer(dataFileTreeCellRendererMap.get(mode));
	}

	public void reloadTreeModelAfterKMeansClustering() {
		GetDataFileTreeWorker worker = new GetDataFileTreeWorker(this, d.getDataFile(), frame);
		worker.execute();
	}
