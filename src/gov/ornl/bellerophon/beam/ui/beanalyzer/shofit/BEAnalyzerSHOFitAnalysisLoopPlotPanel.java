...
import gov.ornl.bellerophon.beam.data.util.SHOFitData;
import gov.ornl.bellerophon.beam.enums.SHOFitLoopPlotType;
import gov.ornl.bellerophon.beam.ui.plot.StaticPlotter;

import javax.swing.*;

public class BEAnalyzerSHOFitAnalysisLoopPlotPanel extends JPanel implements MouseMotionListener, MouseListener{
	...
	private SHOFitLoopPlotType loopPlotType;
	private ArrayList<SHOFitData> shoFitDataList;
	
	public BEAnalyzerSHOFitAnalysisLoopPlotPanel(){
    	...
    	setCurrentData(null, null, SHOFitLoopPlotType.A);
	...
	public void setCurrentData(ArrayList<SHOFitData> shoFitDataList, ArrayList<Double> dcList, SHOFitLoopPlotType loopPlotType){

		this.loopPlotType = loopPlotType;
		this.shoFitDataList = shoFitDataList;
		
		if(shoFitDataList==null){
			return;
		}
		
		titleString = "";
		xtitle = "DC Offset (V)";
		ytitle = loopPlotType.toString();
		...
		double[] xArray = new double[dcList.size()];
		double[] yArray = new double[shoFitDataList.size()];
		
		for(int i=0; i<dcList.size(); i++){
			xArray[i] = dcList.get(i);
		}
		
		for(int i=0; i<shoFitDataList.size(); i++){
			
			SHOFitData sfd = shoFitDataList.get(i);
			switch(loopPlotType){
				...
				case P:
					...
					break;
				case A_SIN_P:
					yArray[i] = sfd.getA()*Math.sin(sfd.getP());
					break;
				case A_COS_P:
					yArray[i] = sfd.getA()*Math.cos(sfd.getP());
					break;
		...
		xmin = getMinValue(x[0]);
		xmax = getMaxValue(x[0]);
