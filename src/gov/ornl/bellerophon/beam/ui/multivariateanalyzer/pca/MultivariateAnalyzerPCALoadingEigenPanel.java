...
public class MultivariateAnalyzerPCALoadingEigenPanel extends JPanel implements ComponentListener, 
																				MouseListener, 
	...
	private MultivariateAnalyzerPCAEigenControlPanel eigenControlPanel;
	private MultivariateAnalyzerPCALoadingControlPanel loadingControlPanel;

	private JScrollPane sp;
	private JPanel graphicsPanel;
	
	private int numPCPerSet, numPC, minPCIndex, setIndex, numPCSets;
	private double eigenGlobalMin, eigenGlobalMax, loadingGlobalMin, loadingGlobalMax;
	
	private ArrayList<MultivariateAnalyzerPCAEigenPlotPanel> plotPanelList;
	private ArrayList<GridPointChartScalePanel> eigenChartScalePanelList;
	private ArrayList<GridPointChartScalePanel> loadingChartScalePanelList;
	private GridPointChartScalePanel selectedLoadingChartScalePanel, selectedEigenChartScalePanel;
	...
	public MultivariateAnalyzerPCALoadingEigenPanel(Frame frame, MultivariateAnalyzerData d, MultivariateAnalyzerPCASetViewPanel setPanel){
	
		this.frame = frame;
		this.d = d;
		this.setPanel = setPanel;
		
		eigenControlPanel = new MultivariateAnalyzerPCAEigenControlPanel(this);
		loadingControlPanel = new MultivariateAnalyzerPCALoadingControlPanel(this);
		...
		panel1.add(loadingControlPanel, "1, 1, f, f");	
		...
		panel2.add(eigenControlPanel, "1, 1, f, f");	
		
		tabbedPane = new JTabbedPane();
		tabbedPane.add("Loading Maps", panel1);
		tabbedPane.add("Eigenvectors", panel2);
	...
	public void setCurrentState(){
		...
		loadingChartScalePanelList = new ArrayList<GridPointChartScalePanel>();
		eigenChartScalePanelList = new ArrayList<GridPointChartScalePanel>();
		eigenControlPanel.setCurrentState(pds.getPCADataList().get(0));
		loadingControlPanel.setZoomSize(getInitialLoadingZoomSize());
		arrangeSet();
	}
	
	private int getInitialLoadingZoomSize(){
	...
	private void createSet(){
		...
		setEigenGlobalLimits();
		setLoadingGlobalLimits();
		...
		loadingChartScalePanelList.clear();
		for(int i=minPCIndex; i<minPCIndex+numPCThisSet; i++){
			...
			chartScalePanel.getChartPanel().showCrossHairs(loadingControlPanel.showCrossHairs());
			chartScalePanel.showColorBars(loadingControlPanel.showColorBars());
			chartScalePanel.showAxis(loadingControlPanel.showAxis());
			chartScalePanel.addMouseListener(this);
			loadingChartScalePanelList.add(chartScalePanel);
		}
		
		if(eigenControlPanel.getMode()==MultivariateAnalyzerPCAEigenPanelMode.PLOT_MODE){
			plotPanelList.clear();
			for(int i=minPCIndex; i<minPCIndex+numPCThisSet; i++){
				MultivariateAnalyzerPCAEigenPlotPanel plotPanel = new MultivariateAnalyzerPCAEigenPlotPanel(i); 
				plotPanel.setCurrentData(pds.getPCADataList().get(i).getVPlotMap().get(eigenControlPanel.getType()));
				plotPanel.setPreferredSize(new Dimension(eigenControlPanel.getZoomSize()*20, eigenControlPanel.getZoomSize()*20));
				plotPanel.addMouseListener(this);
				plotPanelList.add(plotPanel);
			}
		}else if(eigenControlPanel.getMode()==MultivariateAnalyzerPCAEigenPanelMode.CELL_MODE){
			eigenChartScalePanelList.clear();
			for(int i=minPCIndex; i<minPCIndex+numPCThisSet; i++){
				GridPointChartScalePanel chartScalePanel = new GridPointChartScalePanel(i); 
				chartScalePanel.getChartPanel().showSelectedGridHighlight(false);
				chartScalePanel.getChartPanel().setGridPointCellMouseListener(this);
				chartScalePanel.getChartPanel().showCrossHairs(eigenControlPanel.showCrossHairs());
				chartScalePanel.showColorBars(eigenControlPanel.showColorBars());
				chartScalePanel.showAxis(eigenControlPanel.showAxis());
				chartScalePanel.addMouseListener(this);
				eigenChartScalePanelList.add(chartScalePanel);
			}
		}
		
		double[] colArray = new double[setPanel.getNumCols()*2];
		double[] rowArray = new double[setPanel.getNumRows()*2];
		...
		for(int i=0; i<numPCThisSet; i++){
			...
			GridPointChartScalePanel chartScalePanel = loadingChartScalePanelList.get(i);
			graphicsPanel.add(chartScalePanel, col + ", " + row);
			setLoadingChartScalePanelState(chartScalePanel);
			if(loadingControlPanel.showColorBars()){
			...
			if(eigenControlPanel.getMode()==MultivariateAnalyzerPCAEigenPanelMode.PLOT_MODE){
				...
				height = Math.max(height, (int) (plotPanel.getPreferredSize().getHeight()));
				
			}else if(eigenControlPanel.getMode()==MultivariateAnalyzerPCAEigenPanelMode.CELL_MODE){
				
				chartScalePanel = eigenChartScalePanelList.get(i);
				graphicsPanel.add(chartScalePanel, col + ", " + row);
				setEigenChartScalePanelState(chartScalePanel);
				if(eigenControlPanel.showColorBars()){
					width += (int) (chartScalePanel.getChartPanel().getPreferredSize().getWidth() + chartScalePanel.getScalePanel().getPreferredSize().getWidth());
				}else{
					width += (int) (chartScalePanel.getChartPanel().getPreferredSize().getWidth());
				}
				height = Math.max(height, (int) (chartScalePanel.getChartPanel().getPreferredSize().getHeight()));

			}
			
			col++;
			if(col==(setPanel.getNumCols()*2)){
				col = 0;
				row++;
			}
	...
	private void setLoadingGlobalLimits(){
		
		loadingGlobalMin = Double.MAX_VALUE;
		loadingGlobalMax = 0.0;
		
		if(loadingControlPanel.applySigma()){
			
			double sigma = loadingControlPanel.getSigmaValue();
			double mean = getLoadingGlobalMean();
			double stdDev = getLoadingGlobalStandardDeviation(mean);
			...
			for(int i=0; i<=pds.getMaxComponentIndex(); i++){
				...
				for(int j=0; j<uArray.length; j++){
					for(int k=0; k<uArray[0].length; k++){
						...
						loadingGlobalMin = Math.min(loadingGlobalMin, value);
						loadingGlobalMax = Math.max(loadingGlobalMax, value);
					}
				}
			}
	...
	private double getLoadingGlobalMean(){
	...
	private double getLoadingGlobalStandardDeviation(double mean){
		int totalNumberOfValues = 0;
	...
	private double getLoadingMean(int index){
		int totalNumberOfValues = 0;
		double totalValue = 0;
	...
	private double getLoadingStandardDeviation(double mean, int index){
		int totalNumberOfValues = 0;
		double sumOfSquares = 0;
		double[][] uArray = pds.getPCADataList().get(index).getUArray();
		for(int i=0; i<uArray.length; i++){
	...
	private void setLoadingChartScalePanelState(GridPointChartScalePanel chartScalePanel){
		...
		if(loadingControlPanel.applyUniversalRange()){
			min = loadingGlobalMin;
			max = loadingGlobalMax;
		}else{
			if(loadingControlPanel.applySigma()){
				
				double sigma = loadingControlPanel.getSigmaValue();
				double mean = getLoadingMean(index);
				double stdDev = getLoadingStandardDeviation(mean, index);
				double stdDevLowLimit = mean-(sigma*stdDev);
				double stdDevHighLimit = mean+(sigma*stdDev);
		...
		for(int i=1; i<binList.size(); i++){
			Color color = loadingControlPanel.getColorMap().getRGB((binList.get(i)-min)/(max-min));
		...
		for(int i=0; i<uArray.length; i++){
			for(int j=0; j<uArray[0].length; j++){
				double value = uArray[i][j];
				Color color = getColorFromBin(value, binList, colorBinList, binListScaleType, loadingControlPanel.mapOutliers());
		...
		chartScalePanel.getChartPanel().setSquareSize(loadingControlPanel.getZoomSize());
	...
	private void setEigenGlobalLimits(){

		if(eigenControlPanel.getGroup()!=null){
		
			eigenGlobalMin = Double.MAX_VALUE;
			eigenGlobalMax = 0.0;
	
			if(eigenControlPanel.applySigma()){
				
				double sigma = eigenControlPanel.getSigmaValue();
				double mean = getEigenGlobalMean();
				double stdDev = getEigenGlobalStandardDeviation(mean);
				double stdDevLowLimit = mean-(sigma*stdDev);
				double stdDevHighLimit = mean+(sigma*stdDev);
				
				for(int i=0; i<=pds.getMaxComponentIndex(); i++){

					PCADataCell[][] array = pds.getPCADataList().get(i).getVCellMap().get(eigenControlPanel.getGroup());
					for(int j=0; j<array.length; j++){
						for(int k=0; k<array[0].length; k++){
							PCADataCell cell = array[j][k];
							double value = cell.getValueMap().get(eigenControlPanel.getType());
							if(value < stdDevLowLimit || value > stdDevHighLimit){
								continue;
							}
							eigenGlobalMin = Math.min(eigenGlobalMin, value);
							eigenGlobalMax = Math.max(eigenGlobalMax, value);
						}
					}
	...
	private double getEigenGlobalMean(){
		int totalNumberOfValues = 0;
		double totalValue = 0;
		for(int i=0; i<=pds.getMaxComponentIndex(); i++){
			PCADataCell[][] array = pds.getPCADataList().get(i).getVCellMap().get(eigenControlPanel.getGroup());
	...
	private double getEigenGlobalStandardDeviation(double mean){
		int totalNumberOfValues = 0;
		double sumOfSquares = 0;
		for(int i=0; i<=pds.getMaxComponentIndex(); i++){
			PCADataCell[][] array = pds.getPCADataList().get(i).getVCellMap().get(eigenControlPanel.getGroup());
	...
	private double getEigenMean(int index){
		int totalNumberOfValues = 0;
		double totalValue = 0;
		PCADataCell[][] array = pds.getPCADataList().get(index).getVCellMap().get(eigenControlPanel.getGroup());
	...
	private double getEigenStandardDeviation(double mean, int index){
		int totalNumberOfValues = 0;
		double sumOfSquares = 0;
		PCADataCell[][] array = pds.getPCADataList().get(index).getVCellMap().get(eigenControlPanel.getGroup());
		for(int i=0; i<array.length; i++){
	...
	private void setEigenChartScalePanelState(GridPointChartScalePanel chartScalePanel){
		...
		if(eigenControlPanel.applyUniversalRange()){
			min = eigenGlobalMin;
			max = eigenGlobalMax;
		}else{
			if(eigenControlPanel.applySigma()){
				...
				double sigma = eigenControlPanel.getSigmaValue();
				double mean = getEigenMean(index);
				double stdDev = getEigenStandardDeviation(mean, index);
				double stdDevLowLimit = mean-(sigma*stdDev);
				double stdDevHighLimit = mean+(sigma*stdDev);
		...
		max = binList.get(binList.size()-1);
		for(int i=1; i<binList.size(); i++){
			Color color = eigenControlPanel.getColorMap().getRGB((binList.get(i)-min)/(max-min));
			colorBinList.add(color);
		}
		
		PCADataCell[][] valueArray = pds.getPCADataList().get(index).getVCellMap().get(eigenControlPanel.getGroup());
		Color[][] colorArray = new Color[valueArray.length][valueArray[0].length];
		
		for(int i=0; i<valueArray.length; i++){
			for(int j=0; j<valueArray[0].length; j++){
				PCADataCell cell = valueArray[i][j];
				double value = cell.getValueMap().get(eigenControlPanel.getType());
				Color color = getColorFromBin(value, binList, colorBinList, binListScaleType, eigenControlPanel.mapOutliers());
				colorArray[i][j] = color;
			}
		}
		
		chartScalePanel.getChartPanel().setGridWidth(valueArray.length);
		chartScalePanel.getChartPanel().setGridHeight(valueArray[0].length);
		chartScalePanel.getChartPanel().setColorArray(colorArray);
		chartScalePanel.getChartPanel().setSquareSize(eigenControlPanel.getZoomSize());
		chartScalePanel.getChartPanel().addMouseListener(this);
		chartScalePanel.getScalePanel().setCurrentState(binList, 
												colorBinList, 
												new DecimalFormat("0.000E0"), 
	...
	public void gridPointCellPanelMouseOvered(GridPoint gridPoint) {
		...
		if(gridPoint!=null){
			...
			if(selectedEigenChartScalePanel!=null){
			
				double value = pds.getPCADataList().get(selectedEigenChartScalePanel.getIndex()).getVCellMap().get(eigenControlPanel.getGroup())[gridPoint.getX()-1][gridPoint.getY()-1].getValueMap().get(eigenControlPanel.getType());
				double xValue = pds.getPCADataList().get(selectedEigenChartScalePanel.getIndex()).getVCellMap().get(eigenControlPanel.getGroup())[gridPoint.getX()-1][gridPoint.getY()-1].getXValue();
				double yValue = pds.getPCADataList().get(selectedEigenChartScalePanel.getIndex()).getVCellMap().get(eigenControlPanel.getGroup())[gridPoint.getX()-1][gridPoint.getY()-1].getYValue();
				String pc = String.valueOf(selectedEigenChartScalePanel.getIndex()+1);
				...
				eigenControlPanel.setValues(pc, x, y, xValueString, yValueString, valueString);
			
			}else if(selectedLoadingChartScalePanel!=null){
				
				String pc = String.valueOf(selectedLoadingChartScalePanel.getIndex()+1);
				String x = String.valueOf(gridPoint.getX());
				String y = String.valueOf(gridPoint.getY());
				...
				loadingControlPanel.setValues(pc, x, y, value);
				
			}
			
		}else{
			loadingControlPanel.setValues("", "", "", "");
			eigenControlPanel.setValues("", "", "", "", "", "");
		}
	}
		...
		if(me.getSource() instanceof MultivariateAnalyzerPCAEigenPlotPanel){
			for(MultivariateAnalyzerPCAEigenPlotPanel panel: plotPanelList){
				if(panel.equals((MultivariateAnalyzerPCAEigenPlotPanel) me.getSource())){
					...
					eigenControlPanel.setValues(String.valueOf(selectedPlotPanel.getIndex() + 1), "", "", "", "", "");
					tabbedPane.setSelectedIndex(1);
					break;
				}
			}
		}else if(me.getSource() instanceof GridPointChartPanel){
			for(GridPointChartScalePanel panel: eigenChartScalePanelList){
				if(panel.getChartPanel().equals((GridPointChartPanel) me.getSource())){
					selectedEigenChartScalePanel = panel;
					selectedLoadingChartScalePanel = null;
					tabbedPane.setSelectedIndex(1);
					break;
				}
			}
			for(GridPointChartScalePanel panel: loadingChartScalePanelList){
				if(panel.getChartPanel().equals((GridPointChartPanel) me.getSource())){
					selectedEigenChartScalePanel = null;
					selectedLoadingChartScalePanel = panel;
	...
	public void exportCurrentData() {
		...
		String suffix = "_loading_map_eigenvalue_data";
	...
	private void writeExcelReport(){
		...
		MultivariateAnalyzerPCAEigenPanelMode mode = eigenControlPanel.getMode();
    	String group = eigenControlPanel.getGroup();
    	ComplexValueType type = eigenControlPanel.getType();
		...
		for(int i=0; i<numPCThisSet; i++){
			...
			int cellOffset = 0;
			int index;
			GridPointChartScalePanel chartScalePanel;
			Iterator<GridPoint> itr;
			...
			chartScalePanel = loadingChartScalePanelList.get(i);
			...
			rowCounter = sheet.getPhysicalNumberOfRows() + 4;
			row = sheet.createRow(rowCounter++);
			cellCounter = 0;
		    ...
		    switch (mode) {
		    	...
		    	case CELL_MODE:
			    	...
			    	PCADataCell[][] valueArray = pds.getPCADataList().get(selectedEigenChartScalePanel.getIndex()).getVCellMap().get(eigenControlPanel.getGroup());
			    	
			    	for(int j=0; j<valueArray.length; j++){
						for(int k=0; k<valueArray[0].length; k++){
							PCADataCell cellValue = valueArray[j][k];
							double value = cellValue.getValueMap().get(eigenControlPanel.getType());
							int xpos = j+1;
							int ypos = k+1;
	...
	public void exportCurrentImage() {
		...
		String suffix = "_loading_map_eigenvalue";
		String dataFileName = d.getDataFile().getName();
		String filename = "";
