...
public class MultivariateAnalyzerPCAScreePanel extends JPanel implements ChangeListener, 
																			ActionListener, 
																			KeyListener, 
																			ImageExporter,
																			DataExporter,
	...
	private MultivariateAnalyzerPCAScreePlotPanel plotPanel;
	...
	public MultivariateAnalyzerPCAScreePanel(Frame frame, MultivariateAnalyzerData d, GetPCAUVResultsListener gpuvrl){
		...
		plotPanel = new MultivariateAnalyzerPCAScreePlotPanel(this);
		...
		updateButton = Buttons.getIconButton("Update Results"
		...
		double[] columnOptions = {10, TableLayoutConstants.FILL, 10, TableLayoutConstants.PREFERRED, 10};
		...
		optionsPanel.add(updateButton,  "3, 1, 3, 3, f, c");
	...
	public void setCurrentState(){
		...
		this.pds = d.getDataFile().getPCADataSet();

		pcSlider.removeChangeListener(this);
		pcSlider.setMinimum(1);
		pcSlider.setMaximum(pds.getPCADataList().size());
		pcSlider.setValue(pds.getMaxComponentIndex()+1);
		pcSlider.addChangeListener(this);
		
		pcSpinner.removeChangeListener(this);
		pcModel.setMinimum(1);
		pcModel.setMaximum(pds.getPCADataList().size());
		pcModel.setValue(pcSlider.getValue());
		pcSpinner.addChangeListener(this);
		
		valueField.setText(new DecimalFormat("0.000E0").format(pds.getPCADataList().get(pcSlider.getValue()-1).getS()));
		
		plotPanel.setCurrentData(pds);
	...
	private boolean goodPCCutOff(){
		...
		try{
			...
			if(cutoffValue<1 ||  cutoffValue>pds.getPCADataList().size()){
				AttentionDialog.createDialog(frame, "Please enter an integer value between 1 and " + pds.getPCADataList().size() + ".");
				goodPCCutOff = false;
			}
	...
	public void actionPerformed(ActionEvent ae){
		if(ae.getSource()==scaleBoxX || ae.getSource()==scaleBoxY){
			...
			pds.setMaxComponentIndex(pcSlider.getValue()-1);
			GetPCAUVResultsWorker worker = new GetPCAUVResultsWorker(gpuvrl, pds, frame);
			worker.execute();
		}
	...
	private void writeExcelReport(){
	    ...
	    int kmax = pds.getPCADataList().size();
	    double[] x = new double[kmax];
	    double[] y = new double[kmax];
	    
	    for(int i=1; i<=kmax; i++){
			x[i-1] = i;
			y[i-1] = pds.getPCADataList().get(i-1).getS();
		}
