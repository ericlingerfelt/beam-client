...
public class ImageProcessorPCAImageCleaningFFTPanel extends JPanel implements ComponentListener, 
																				GridPointCellPanelMouseListener,
																				ImageProcessorPCAImageCleaningFFTControlPanelListener,
	...
	private TreeMap<PCAImageType, ImageProcessorPCAImageCleaningFFTControlPanel> controlPanelMap;
	...
	public ImageProcessorPCAImageCleaningFFTPanel(Frame frame, ImageProcessorData d){
		this.frame = frame;
		this.d = d;

		scalePanelMap = new TreeMap<PCAImageType, GridPointChartScalePanel>();
		scalePanelMap.put(PCAImageType.CLEAN_FFT, new GridPointChartScalePanel(PCAImageType.CLEAN_FFT.ordinal()));
		scalePanelMap.put(PCAImageType.NOISE_FFT, new GridPointChartScalePanel(PCAImageType.NOISE_FFT.ordinal()));
		
		controlPanelMap = new TreeMap<PCAImageType, ImageProcessorPCAImageCleaningFFTControlPanel>();
		controlPanelMap.put(PCAImageType.CLEAN_FFT, new ImageProcessorPCAImageCleaningFFTControlPanel(this, PCAImageType.CLEAN_FFT));
		controlPanelMap.put(PCAImageType.NOISE_FFT, new ImageProcessorPCAImageCleaningFFTControlPanel(this, PCAImageType.NOISE_FFT));
		...
		panel1.add(controlPanelMap.get(PCAImageType.CLEAN_FFT), "1, 1, f, f");	
		...
		panel2.add(controlPanelMap.get(PCAImageType.NOISE_FFT), "1, 1, f, f");	
		
		tabbedPane = new JTabbedPane();
		tabbedPane.add(PCAImageType.CLEAN_FFT.toString(), panel1);
		tabbedPane.add(PCAImageType.NOISE_FFT.toString(), panel2);
	...
	public void setCurrentState(){
		...
		scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().showSelectedGridHighlight(false);
		scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().setGridPointCellMouseListener(this);
		scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().showCrossHairs(controlPanelMap.get(PCAImageType.CLEAN_FFT).showCrossHairs());
		scalePanelMap.get(PCAImageType.CLEAN_FFT).showColorBars(controlPanelMap.get(PCAImageType.CLEAN_FFT).showColorBars());
		scalePanelMap.get(PCAImageType.CLEAN_FFT).showAxis(controlPanelMap.get(PCAImageType.CLEAN_FFT).showAxis());
		scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().addMouseListener(this);
		
		scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().showSelectedGridHighlight(false);
		scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().setGridPointCellMouseListener(this);
		scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().showCrossHairs(controlPanelMap.get(PCAImageType.NOISE_FFT).showCrossHairs());
		scalePanelMap.get(PCAImageType.NOISE_FFT).showColorBars(controlPanelMap.get(PCAImageType.NOISE_FFT).showColorBars());
		scalePanelMap.get(PCAImageType.NOISE_FFT).showAxis(controlPanelMap.get(PCAImageType.NOISE_FFT).showAxis());
		scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().addMouseListener(this);

		graphicsPanel.removeAll();
		graphicsPanel.add(scalePanelMap.get(PCAImageType.CLEAN_FFT), "1, 1, c, c");
		graphicsPanel.add(scalePanelMap.get(PCAImageType.NOISE_FFT), "3, 1, c, c");
		
		setChartScalePanelState(PCAImageType.CLEAN_FFT);
		setChartScalePanelState(PCAImageType.NOISE_FFT);
		
		int width = 0;
		if(controlPanelMap.get(PCAImageType.CLEAN_FFT).showColorBars()){
			width += (int) (scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().getPreferredSize().getWidth() + scalePanelMap.get(PCAImageType.CLEAN_FFT).getScalePanel().getPreferredSize().getWidth());
		}else{
			width += (int) (scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().getPreferredSize().getWidth());
		}
		if(controlPanelMap.get(PCAImageType.NOISE_FFT).showColorBars()){
			width += (int) (scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().getPreferredSize().getWidth() + scalePanelMap.get(PCAImageType.NOISE_FFT).getScalePanel().getPreferredSize().getWidth());
		}else{
			width += (int) (scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().getPreferredSize().getWidth());
		}
		int height = (int) (scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().getPreferredSize().getHeight());
	...
	public void fftControlPanelZoomSizeChanged(PCAImageType type) {
		if(type==PCAImageType.CLEAN_FFT){
			controlPanelMap.get(PCAImageType.NOISE_FFT).setZoomSize(controlPanelMap.get(PCAImageType.CLEAN_FFT).getZoomSize());
		}else if(type==PCAImageType.NOISE_FFT){
			controlPanelMap.get(PCAImageType.CLEAN_FFT).setZoomSize(controlPanelMap.get(PCAImageType.NOISE_FFT).getZoomSize());
		}
		setChartScalePanelState(PCAImageType.CLEAN_FFT);
		setChartScalePanelState(PCAImageType.NOISE_FFT);
	}
	...
	private void setChartScalePanelState(PCAImageType type){
		...
		ImageProcessorPCAImageCleaningFFTControlPanel controlPanel = controlPanelMap.get(type);
		...
		ChartScaleType binListScaleType = controlPanel.getChartScaleType();
		...
		height = (int) (scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().getPreferredSize().getHeight());
	...
	public void gridPointCellPanelMouseOvered(GridPoint gridPoint) {
		if(gridPoint!=null && selectedScalePanel!=null){
			...
			if(selectedScalePanel.equals(scalePanelMap.get(PCAImageType.CLEAN_FFT))){
				String value = new DecimalFormat("0.000E0").format(picds.getImage(PCAImageType.CLEAN_FFT).getValueArray()[gridPoint.getX()-1][gridPoint.getY()-1]);
				controlPanelMap.get(PCAImageType.CLEAN_FFT).setValues(x, y, value);
			}else if(selectedScalePanel.equals(scalePanelMap.get(PCAImageType.NOISE_FFT))){
				String value = new DecimalFormat("0.000E0").format(picds.getImage(PCAImageType.NOISE_FFT).getValueArray()[gridPoint.getX()-1][gridPoint.getY()-1]);
				controlPanelMap.get(PCAImageType.NOISE_FFT).setValues(x, y, value);
			}
		}else{
			controlPanelMap.get(PCAImageType.CLEAN_FFT).setValues("", "", "");
			controlPanelMap.get(PCAImageType.NOISE_FFT).setValues("", "", "");
		}
	}
	...
	public void mouseEntered(MouseEvent me){
		if(me.getSource() instanceof GridPointChartPanel){
			if(scalePanelMap.get(PCAImageType.CLEAN_FFT).getChartPanel().equals((GridPointChartPanel) me.getSource())){
				selectedScalePanel = scalePanelMap.get(PCAImageType.CLEAN_FFT);
				tabbedPane.setSelectedIndex(0);
			}else if(scalePanelMap.get(PCAImageType.NOISE_FFT).getChartPanel().equals((GridPointChartPanel) me.getSource())){
				selectedScalePanel = scalePanelMap.get(PCAImageType.NOISE_FFT);
