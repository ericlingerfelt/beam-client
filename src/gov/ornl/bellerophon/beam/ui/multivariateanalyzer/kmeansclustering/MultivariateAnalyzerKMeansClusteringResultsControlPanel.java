...
public class MultivariateAnalyzerKMeansClusteringResultsControlPanel extends JPanel implements ActionListener, ChangeListener{

	private JSpinner zoomSpinner;
	private SpinnerNumberModel zoomModel;
	private JCheckBox viewColorBarsBox, viewAxesBox, showCrossHairsBox;
	private JTextField xField, yField, valueField;
	private JComboBox<ColorMapType> colorMapBox;
	private MultivariateAnalyzerKMeansClusteringResultsControlPanelListener listener;
	
	public MultivariateAnalyzerKMeansClusteringResultsControlPanel(MultivariateAnalyzerKMeansClusteringResultsControlPanelListener listener){
	...
	public void stateChanged(ChangeEvent ce) {
		if(ce.getSource()==zoomSpinner){
			listener.resultsControlPanelZoomSizeChanged();
		}
	}
	
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==viewColorBarsBox){
			listener.resultsControlPanelShowColorBarsChanged();
		}else if(ae.getSource()==viewAxesBox){
			listener.resultsControlPanelShowAxisChanged();
		}else if(ae.getSource()==showCrossHairsBox){
			listener.resultsControlPanelShowCrossHairsChanged();
		}else if(ae.getSource()==colorMapBox){
			listener.resultsControlPanelColorMapChanged();
		}
	}
