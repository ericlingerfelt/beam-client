...
public class ImageProcessorPCAImageCleaningScreePlotPanel extends JPanel implements MouseMotionListener, MouseListener, ComponentListener{
	...
	public ImageProcessorPCAImageCleaningScreePlotPanel(ImageProcessorPCAImageCleaningScreePanel owner){
	...
	public void setCurrentData(PCAImageCleaningDataSet picds){
		...
		int size = 1;
		imax = size;
		kmax = picds.getPCADataList().size();
		...
		for(int i=1; i<=picds.getPCADataList().size(); i++){
			x[0][i-1] = i;
			y[0][i-1] = picds.getPCADataList().get(i-1).getS();
		}
		...
		xmin = 1;
		xmax = picds.getPCADataList().size();
