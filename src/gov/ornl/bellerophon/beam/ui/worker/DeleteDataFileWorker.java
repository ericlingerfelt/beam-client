 ...
 * 
 * Filename: DeleteDataFileWorker.java
...
public class DeleteDataFileWorker extends SwingWorker<ErrorResult, Void>{

	private Window owner;
	private DeleteDataFileListener ddfl;
	 ...
	 * @param node the node
	 */
	public DeleteDataFileWorker(DeleteDataFileListener ddfl
								, DataFile dataFile
								, Window owner){
		this.ddfl = ddfl;
		this.owner = owner;
	...
	protected ErrorResult doInBackground(){
		...
		return WebServiceCom.getInstance().doWebServiceComCall(dataFile, Action.DELETE_DATA_FILE);
	}
	...
	protected void done(){
		try{
			...
			if(!result.isError()){
				ddfl.updateAfterDeleteDataFile(dataFile);
