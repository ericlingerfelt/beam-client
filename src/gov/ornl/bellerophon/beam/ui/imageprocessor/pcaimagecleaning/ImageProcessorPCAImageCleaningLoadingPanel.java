...
public class ImageProcessorPCAImageCleaningLoadingPanel extends JPanel implements ComponentListener, 
																				GridPointCellPanelMouseListener,
																				MouseListener, 
																				ImageProcessorPCAImageCleaningSetViewPanelListener, 
																				ImageProcessorPCAImageCleaningLoadingControlPanelListener,
																				ImageExporter,
																				DataExporter,
																				ExcelWriter{
	...
	private ImageProcessorPCAImageCleaningLoadingControlPanel controlPanel;
	...
	public ImageProcessorPCAImageCleaningLoadingPanel(Frame frame, ImageProcessorData d, ImageProcessorPCAImageCleaningSetViewPanel setPanel){
		this.frame = frame;
		this.d = d;
		this.setPanel = setPanel;

		controlPanel = new ImageProcessorPCAImageCleaningLoadingControlPanel(this);
	...
	private int getInitialZoomSize(){
		...
		int zoomSize = (int) Math.ceil( 250 / (double) picds.getPCADataList().get(0).getUArray()[0].length);
		return zoomSize;
	...
	public void arrangeSet(){
		...
		numPC = picds.getPCADataList().size();
...
private void setGlobalLimits(){
		...
		if(controlPanel.applySigma()){
			...
			for(int i=0; i<picds.getPCADataList().size(); i++){
				double[][] uArray = picds.getPCADataList().get(i).getUArray();
	...
	private double getMean(int index){
		int totalNumberOfValues = 0;
		double totalValue = 0;
		double[][] uArray = picds.getPCADataList().get(index).getUArray();
	...
	public void gridPointCellPanelMouseOvered(GridPoint gridPoint) {
		if(gridPoint!=null && selectedChartScalePanel!=null){
			...
			String value = new DecimalFormat("0.000E0").format(picds.getPCADataList().get(Integer.valueOf(pc)-1).getUArray()[gridPoint.getX()-1][gridPoint.getY()-1]);
	...
	public void exportCurrentData() {
		...
		String suffix = "_loading_maps_data";
