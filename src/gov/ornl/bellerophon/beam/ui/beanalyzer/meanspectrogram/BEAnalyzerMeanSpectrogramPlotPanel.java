...
public class BEAnalyzerMeanSpectrogramPlotPanel extends JPanel implements MouseMotionListener, MouseListener, ComponentListener{
	...
	private BEAnalyzerMeanSpectrogramAnalysisPanel owner;
	
	public BEAnalyzerMeanSpectrogramPlotPanel(BEAnalyzerMeanSpectrogramAnalysisPanel owner){
		this.owner = owner;
	...
	public void setCurrentData(MeanSpectrogramData msd, ComplexValueType complexValueType){
		...
		for(int i=0; i<msd.getYArray().length; i++){
			x[0][i] = msd.getYArray()[i];
			y[0][i] = msd.getZArrayMap().get(complexValueType)[0][i];
		}
		...
		xmin = msd.getYArray()[0];
		xmax = msd.getYArray()[msd.getYArray().length-1];
